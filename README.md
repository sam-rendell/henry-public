...
Snapshot of 2026-09-20T04:17:47Z. Rebuilt daily.
...
|---|---:|---:|---|
| `reports` | 17,921 | 10 MB | Threat reports: metadata, our summary, and the structured fields extracted from each. |
| `texts` | 14,647 | 167 MB | Full document text, one JSON object per line, sharded by year. `henry-texts-feed-references` is different in kind: pages fetched from indicator feed URLs, including CDN assets and parked domains. Keep them apart in any analysis. |
| `extractions` | 14,760 | 17 MB | The full structured extraction per report: victimology, MITRE techniques, malware, infrastructure, hunting guidance and the analytical reasoning behind them. |
| `indicators` | 168,219 | 23 MB | Network, file and on-chain indicators. `disposition` says what each one IS -- see below; do not build a blocklist without reading it. |
| `indicator_reports` | 69,009 | 10 MB | Which indicator appeared in which report, with the provenance tier of that link. |
| `incidents` | 1,761 | 2 MB | Curated incidents, deduplicated. Several reports of one event are merged; `merged_count` and `merged_from` say so. |
| `events` | 4,824 | 3 MB | The channel stream as filed, one row per report, undeduplicated. |
| `artefacts` | 72,841 | 10 MB | Hunting artefacts pulled from report prose: mutexes, paths, filenames, registry keys, yara/sigma terms. |
...
summary for all 17,921 reports, and
mirrored text for 14,647 documents.
